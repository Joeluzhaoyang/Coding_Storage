...
{
    if(strcmp(name,dir->name)==0|| ( strcmp(name,(dir->subfile)->name)==0 && dir->subfile!=nullptr) || ( strcmp(name,((dir->subdir)->name))==0 &&(dir->subdir)!= nullptr))
    {
        return false;
    }
    if(dir->subdir!=NULL)
    {
        int xxxx=1;//planned recurrsion function here
    }
    return true;
}
...
int deleteFile(Dir *dir, const char *name)
{
    ...
    if(!confcheck(dir,name))
    {return 2;}
    if(strcmp(dir->subfile->name,name))
    {
        File* change=nullptr;
...
int deleteDir(Dir *dir, const char *name, bool recursive)
{
    if(dir==nullptr || name==nullptr)
    {return 1;}
    if((dir->subdir!=nullptr || dir->subfile!=nullptr)&&(recursive==false))
    {
        return 3;
    }
}
