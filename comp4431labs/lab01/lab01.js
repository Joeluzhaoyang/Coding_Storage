...
function handleNoteOn(key_number) {
     ...
     */
    //*
    let result_mode = $(":radio[name=play-mode]:checked").val();
    if(result_mode == "major"){
        MIDI.noteOn(0,pitch+4,amplitude);
        MIDI.noteOn(0,pitch+7,amplitude);
        console.log("Major!");
    }
    else if(result_mode == "minor"){
...
function handleInstrument(evt){
    let yueqi_number = parseInt($("#inputGroupSelect01").val());
    console.log(yueqi_number);
    MIDI.programChange(0, yueqi_number);
}
...
$(document).ready(function() {
    MIDI.loadPlugin({
        ...
        onsuccess: function() {
             ...
             */
            addEventListener("input", handleInstrument);      
          }
    });
});
