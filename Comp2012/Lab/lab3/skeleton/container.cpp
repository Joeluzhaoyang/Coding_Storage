...
void Container::display() const // Task 2 - To Do
{
    ...
    for(;num_container < _num_container;num_container++)
    {_containers[num_container]->display();}
    for(;num_textbox < _num_textBox;num_textbox++)
    {_textBoxes[num_textbox]->display();}
    for(;num_buttom < _num_button;num_buttom++)
    {_buttons[num_buttom]->display();}
}