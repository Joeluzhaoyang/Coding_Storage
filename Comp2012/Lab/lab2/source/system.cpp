...
Match::Match(const char* first_country_name, const char* second_country_name, const char** first_players, int num_first_players, const char** second_players, int num_second_players){
    for(int i = 0 ; i<30 ; i++)//Events
    {this->events[i]=nullptr;}
    this->num_event = 0;

    this->first_country_name = new char[strlen(first_country_name)+1];//Country name
    ...
    this->num_first_players = num_first_players;//Number of player
    this->num_second_players = num_second_players;

    this->first_players = new char*[11];//players
    for(int i = 0 ; i<11 ; i++)
    {
...
Match::Match(const Match& match){
    ...
    for(int i = 0 ; i<match.num_event ; i++)//Events
    {
        this->events[i] = new Event(*match.events[i]);
        this->num_event++;
    }

    this->first_country_name = new char[strlen(match.first_country_name)+1];//Country name
    ...
    this->num_first_players = match.num_first_players;//Number of player
